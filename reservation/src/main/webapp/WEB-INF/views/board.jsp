...
function commentlist(list){
	...
	for(var i=0;i<Object.keys(list).length;i++){
		$('.comment_table').append("<tr class='comment_content'id='comment_tr"+list[i].commentSeq+"'>");
		$('#comment_tr'+list[i].commentSeq).append("<td class='"+list[i].commentSeq+"'><a class='commentWriter'>작성자:"+list[i].writer+"</a><a class='comment_regate'>날짜:"+list[i].regDate+"</a> <img class='dat_update_icon' src='${path}/img/icon/edit.png'>  <img class='dat_delete_icon'src='${path}/img/icon/delete.png'> <br/><p>"+list[i].content+"</p>");
		$('.'+list[i].commentSeq).append("</tr>");
		if(list[i].modified=='Y'){
			$('.'+list[i].commentSeq).children().eq(1).append('<a class="comment_modified">수정됨</a>');
		}
	}
	if(i!=0){
		lastcomment=list[i-1].commentSeq;
		lastcomment_top = $('.'+list[i-1].commentSeq).offset();
	}	
}
//댓글 불러오는 함수
...
function tableload(list){
	...
	for(var i=0;i<Object.keys(list).length;i++)
		...
		if(list[i].security=="Y"){
			$('#'+list[i].seq).append('<td class="title">'+list[i].title+'<img src="${path}/img/icon/padlock.png" style="margin-left:3; width: 12px;"></td>');
		}else{
...
$(document).ready(function(){
	...
	$(document).on('click','.dat_update_submit',function(){
		...
		$.ajax({
			...
			success:function(result){
				if(result){
					$('.'+lastdatseq).html("<a class='commentWriter'>작성자:"+result.writer+"</a><a class='comment_reg_date'>날짜:"+result.regDate+"</a> <img class='dat_update_icon' src='${path}/img/icon/edit.png'>  <img class='dat_delete_icon'src='${path}/img/icon/delete.png'> <br/><p>"+result.content+"</p>");	
...
Kakao.init('8df52c0b34a125567780d87840e6300f');

function plusFriendChat() {
  Kakao.PlusFriend.chat({
    plusFriendId: '_SxfxhxfT' 
